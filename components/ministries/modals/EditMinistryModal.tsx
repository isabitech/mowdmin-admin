...
import { Ministry, UpdateMinistryData, MINISTRY_TYPE_CONFIG } from '@/constant/ministryTypes';

interface EditMinistryModalProps {
  isOpen: boolean;
  onClose: () => void;
  ministry: Ministry;
  onSuccess: (data: UpdateMinistryData) => void;
...
export default function EditMinistryModal({
  ...
  const [formData, setFormData] = useState<UpdateMinistryData>({
    ...
    contactInfo: {
      primaryContact: ministry.contactInfo?.primaryContact || '',
      email: ministry.contactInfo?.email || '',
      phone: ministry.contactInfo?.phone || '',
    },
    budget: ministry.budget ? { ...ministry.budget } : {
      allocated: 0,
      currency: 'USD',
      period: 'yearly',
    },
    targetAudience: ministry.targetAudience ? { ...ministry.targetAudience } : {
      gender: 'mixed',
  ...
  const handleInputChange = (field: keyof UpdateMinistryData, value: any) => {
  ...
  const validateForm = () => {
    ...
    if (formData.budget && typeof formData.budget.allocated === 'number' && formData.budget.allocated < 0) {
          ...
          <div className="p-8 border-b border-gray-100 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900 tracking-tight">Edit Ministry</h2>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-0.5">Update Ministry Profile</p>
                </div>
              </div>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Describe the ministry's core mission..."
                  />
                </div>
              </div>
            </section>
