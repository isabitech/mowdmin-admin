...
interface CreateBibleStoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (data: CreateBibleStoryData) => void;
  isSubmitting: boolean;
}

export default function CreateBibleStoryModal({
  isOpen,
  onClose,
  onSuccess,
  isSubmitting,
}: CreateBibleStoryModalProps) {
  const [formData, setFormData] = useState<CreateBibleStoryData>({
    title: '',
    summary: '',
    content: '',
    book: 'Genesis',
  ...
  const handleInputChange = (field: keyof CreateBibleStoryData, value: any) => {
  ...
  const validateForm = () => {
    ...
    if (formData.chapter < 1) newErrors.chapter = 'Chapter must be at least 1';
    if (formData.verseStart < 1) newErrors.verseStart = 'Start verse must be at least 1';
    if (formData.verseEnd < formData.verseStart) newErrors.verseEnd = 'End verse must be >= start verse';
    if (formData.ageGroups.length === 0) newErrors.ageGroups = 'At least one age group is required';
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900">Create New Bible Story</h2>
              <button
                type="button"
                onClick={onClose}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {Object.entries(AGE_GROUPS).map(([key, config]) => (
                  <label key={key} className="flex items-center">
                    <input
                      type="checkbox"
                      checked={formData.ageGroups.includes(key as any)}
